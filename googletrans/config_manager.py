...
def ayarlari_kaydet():
    # DEĞİŞTİ: api_anahtari kaldırıldı.
    config['Genel'] = {'tesseract_yolu': AYARLAR['tesseract_yolu'], 'arayuz_dili': AYARLAR['arayuz_dili'], 'hedef_dil': AYARLAR['hedef_dil'], 'baslangicta_baslat': str(AYARLAR['baslangicta_baslat'])}
...
def ayarlari_yukle():
    ...
    if not os.path.exists(CONFIG_DOSYASI):
        # DEĞİŞTİ: api_anahtari kaldırıldı.
        config['Genel'] = {'tesseract_yolu': '', 'arayuz_dili': 'TR', 'hedef_dil': 'TR', 'baslangicta_baslat': 'True'}
    ...
    AYARLAR = {
        ...
        'kaynak_metin_benzerlik_esigi': config.getfloat('Arayuz', 'kaynak_metin_benzerlik_esigi', fallback=0.9),
        'kaynak_metin_min_uzunluk': config.getint('Arayuz', 'kaynak_metin_min_uzunluk', fallback=3),
