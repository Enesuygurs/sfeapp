...
import mss
import pytesseract
from googletrans import Translator # DEĞİŞTİ: deepl yerine googletrans
import keyboard
import pystray
...
def main_translation_loop():
    ...
    try:
        # DEĞİŞTİ: Translator artık API anahtarı olmadan başlatılıyor.
        translator = Translator()
    except Exception as e:
        print(f"HATA: Google Translator oluşturulamadı: {e}.")
        gui_queue.put({'type': 'show_message_error', 'title': get_lang('error_title_translator_init'), 'body': get_lang('error_body_translator_init')})
        translator = None
                    ...
                    if yeni_normal_metin:
                        ...
                        if benzerlik < AYARLAR['kaynak_metin_benzerlik_esigi']:
                            ...
                            if translator:
                                try:
                                    if not is_paused:
                                        # DEĞİŞTİ: Çeviri metodu ve parametre adı güncellendi.
                                        cevirilmis = translator.translate(yeni_normal_metin, dest=AYARLAR['hedef_dil'])
                                        if not is_paused:
