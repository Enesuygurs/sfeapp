...
class GuiManager:
    ...
    def process_queue(self):
        try:
            ...
            # DEĞİŞTİRİLDİ: Alan seçici mantığı artık daha akıllı
            elif msg_type == 'open_selector':
                # Mesajdan, işlem sonrası devam edilip edilmeyeceği bilgisini al
                should_resume = message.get('should_resume', False)
                
                # Alan seçici penceresini oluştur
                selector = AlanSecici(self.root)
                
                # Pencerenin kapanmasını bekle (bu satır, kodun burada durmasını sağlar)
                self.root.wait_window(selector)
                
                # Pencere kapandıktan sonra, eğer devam edilmesi gerekiyorsa,
                # ana dosyadan aldığımız 'toggle' fonksiyonunu çağır.
                if should_resume:
...
# Geri kalan kodlar (AyarlarPenceresi, AlanSecici, OverlayGUI) aynı kalabilir.
# Değişiklik sadece yukarıdaki GuiManager sınıfındaydı.
# Okunabilirlik için sınıfları tam olarak ekliyorum.
...
class AyarlarPenceresi(tk.Toplevel):
    ...
    def kaydet(self):
        ...
        yeni_ayarlar = {
            ...
            'ceviri_omru': float(self.var_ceviri_omru.get()),
        ...
        AYARLAR.update({k: v for k, v in yeni_ayarlar.items() if k in AYARLAR})
...
class AlanSecici(tk.Toplevel):
    ...
    def on_mouse_release(self, event):
        ...
        # Alanın minimum bir boyutta olduğundan emin olalım
    ...
    def update_display_loop(self):
        ...
        if not self.active_translations:
            ...
            for translation in self.active_translations:
                label = tk.Label(
                    self.container, text=translation['text'],
                    font=("Arial", AYARLAR['font_boyutu'], "bold"),
